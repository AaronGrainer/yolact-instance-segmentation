...
class Anchor(object):
  def __init__(self, img_size, feature_map_size, aspect_ratio, scale):
    ...
    self.num_anchors, self.anchors = self._generate_anchors(
        img_size, feature_map_size, aspect_ratio, scale)
    ...
    for idx, f_size in enumerate(feature_map_size):
      ...
      for j, i in product(range(f_size), range(f_size)):
        ...
        for ars in aspect_ratio:
          ...
          ymax = y + (h / 2)
          xmax = x + (w / 2)
          prior_boxes += [ymin * img_size, xmin *
              img_size, ymax * img_size, xmax * img_size]
        count_anchor += 1
  ...
  def _pairwise_intersection(self, gt_bbox):
    ...
    ymin_anchor, xmin_anchor, ymax_anchor, xmax_anchor = tf.unstack(
        self.anchors, axis=-1)
    ymin_gt, xmin_gt, ymax_gt, xmax_gt = tf.unstack(gt_bbox, axis=-1)

    # Calculate intersection
    all_pairs_max_xmin = tf.math.maximum(tf.expand_dims(
        xmin_anchor, axis=-1), tf.expand_dims(xmin_gt, axis=0))
    all_pairs_min_xmax = tf.math.minimum(tf.expand_dims(
        xmax_anchor, axis=-1), tf.expand_dims(xmax_gt, axis=0))
    all_pairs_max_ymin = tf.math.maximum(tf.expand_dims(
        ymin_anchor, axis=-1), tf.expand_dims(ymin_gt, axis=0))
    all_pairs_min_ymax = tf.math.minimum(tf.expand_dims(
        ymax_anchor, axis=-1), tf.expand_dims(ymax_gt, axis=0))
  ...
  def _pairwise_iou(self, gt_bbox):
    ...
    pairwise_area = tf.expand_dims(
        area_anchor, axis=-1) + tf.expand_dims(area_gt, axis=0)
  ...
  def matching(self, threshold_pos, threshold_neg, gt_bbox, gt_labels):
    ...
    forced_update_indice = tf.expand_dims(
        tf.math.argmax(pairwise_iou, axis=0), axis=-1)
    max_iou_for_anchors = tf.tensor_scatter_nd_update(
        max_iou_for_anchors, forced_update_indice, forced_update_iou)
    max_id_for_anchors = tf.tensor_scatter_nd_update(
        max_id_for_anchors, forced_update_indice, forced_update_id)
    ...
    max_iou_for_anchors = tf.tensor_scatter_nd_update(
        max_iou_for_anchors, pos_iou, tf.ones(tf.size(pos_iou)))
    neg_iou = tf.where(max_iou_for_anchors < threshold_neg)
    max_iou_for_anchors = tf.tensor_scatter_nd_update(
        max_iou_for_anchors, neg_iou, tf.zeros(tf.size(neg_iou)))
    neu_iou = tf.where(
        tf.math.logical_and((max_iou_for_anchors <= threshold_pos), max_iou_for_anchors >= threshold_neg))
    max_iou_for_anchors = tf.tensor_scatter_nd_update(
        max_iou_for_anchors, neu_iou, -1 * tf.ones(tf.size(neu_iou)))
    ...
    """
    target_cls = tf.multiply(
        tf.cast(match_labels, tf.float32), match_positiveness)
    ...
    center_anchors = tf.stack(
        [self.anchors[:, 1] + (w / 2), self.anchors[:, 0] + (h / 2), w, h], axis=-1)

    # center_gt = tf.map_fn(lambda x: map_to_center_form(x), map_loc)
    h = map_loc[:, 2] - map_loc[:, 0]
    w = map_loc[:, 3] - map_loc[:, 1]
    center_gt = tf.stack(
        [map_loc[:, 1] + (w / 2), map_loc[:, 0] + (h / 2), w, h], axis=-1)
    variances = [0.1, 0.2]
