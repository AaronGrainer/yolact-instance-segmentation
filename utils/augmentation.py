...
def geometric_distortion(img, bboxes, masks, output_size, proto_output_size, classes):
  ...
  v = tf.stack([distort_bbox[0], distort_bbox[1],
                distort_bbox[0], distort_bbox[1]])
  bboxes = bboxes - v
  ...
  # Filter out
  scores = utils.bboxes_intersection(
  ...
  cropped_image = tf.image.resize(
      cropped_image, [output_size, output_size], method=tf.image.ResizeMethod.BILINEAR)
...
def random_augmentation(img, bboxes, masks, output_size, proto_output_size, classes):
  ...
  if FLAG_GEO_DISTORTION > 0.5:
    img, bboxes, masks, classes = geometric_distortion(
        img, bboxes, masks, output_size, proto_output_size, classes)
