...
class Yolact_LearningRateSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
  ...
  def __call__(self, step):
    ...
    def f0(): return (lr - self.warmup_lr) * \
        (step / self.warmup_step) + self.warmup_lr
    def f1():return 1e-3
    def f2():return 1e-4
    def f3():return 1e-5
    def f4():return 1e-6
    ...
    learning_rate = tf.case([(tf.math.logical_and(tf.math.less(warmup_steps, step),
                                                  tf.less_equal(step, 280000.)), f1),
                             (tf.math.logical_and(tf.math.less(280000., step),
                                                  tf.less_equal(step, 600000.)), f2),
                             (tf.math.logical_and(tf.math.less(600000., step),
                                                  tf.less_equal(step, 700000.)), f3),
                             (tf.math.logical_and(tf.math.less(700000., step),
                                                  tf.less_equal(step, 750000.)), f4),
